<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    /**
     * Run the migrations.
     * Remove 'virtual' from courses.delivery_type enum
     */
    public function up(): void
    {
        $driver = DB::getDriverName();
        
        if ($driver === 'mysql' || $driver === 'mariadb') {
            // Remove 'virtual' from enum, keeping 'onsite', 'online', 'hybrid'
            DB::statement("ALTER TABLE `courses` MODIFY COLUMN `delivery_type` ENUM('onsite', 'online', 'hybrid') NOT NULL DEFAULT 'online'");
        } else {
            // For other databases, use Schema builder
            Schema::table('courses', function (Blueprint $table) {
                $table->enum('delivery_type', ['onsite', 'online', 'hybrid'])
                    ->default('online')
                    ->change();
            });
        }
    }

    /**
     * Reverse the migrations.
     * Add 'virtual' back to the enum
     */
    public function down(): void
    {
        $driver = DB::getDriverName();
        
        if ($driver === 'mysql' || $driver === 'mariadb') {
            // Add 'virtual' back to enum
            DB::statement("ALTER TABLE `courses` MODIFY COLUMN `delivery_type` ENUM('onsite', 'online', 'virtual', 'hybrid') NOT NULL DEFAULT 'online'");
        } else {
            Schema::table('courses', function (Blueprint $table) {
                $table->enum('delivery_type', ['onsite', 'online', 'virtual', 'hybrid'])
                    ->default('online')
                    ->change();
            });
        }
    }
};
