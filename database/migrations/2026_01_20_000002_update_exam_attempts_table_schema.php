...
return new class extends Migration
{
    ...
    public function down(): void
    {
        ...
        if (Schema::hasColumn('exam_attempts', 'status')) {
            DB::table('exam_attempts')->where('status', 'submitted')->update(['status' => 'completed']);
            DB::statement("ALTER TABLE exam_attempts MODIFY COLUMN status ENUM('in_progress', 'completed', 'graded') DEFAULT 'in_progress'");
        }
    }
};